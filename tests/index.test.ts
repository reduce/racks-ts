...
import { APIPromise } from 'racks/core/api-promise';

import util from 'node:util';
import Racks from 'racks';
import { APIUserAbortError } from 'racks';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['RACKS_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: Racks) => {
  ...
  test('default logLevel is warn', async () => {
    const client = new Racks({ apiKey: 'My API Key' });
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    const client = new Racks({ logger: logger, apiKey: 'My API Key' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new Racks({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'RACKS_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Racks({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['RACKS_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Racks({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('empty env variable', () => {
      ...
      const client = new Racks({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://racks.cash')
    });
    ...
    test('env variable with environment', () => {
      ...
      expect(
        () =>
          new Racks({ apiKey: 'My API Key', environment: 'production' })
    ...
    test('in request options overridden by client options', () => {
      const client = new Racks({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual('http://localhost:5000/client/foo');
    });

    test('in request options overridden by env variable', () => {
      process.env['RACKS_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Racks({ apiKey: 'My API Key' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Racks({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Racks({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Racks({ apiKey: 'My API Key', fetch: testFetch });
