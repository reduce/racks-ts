...
import { LeaderboardEntry, V1, V1RetrieveLeaderboardParams, V1RetrieveLeaderboardResponse } from './resources/v1/v1';
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['RACKS_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['RACKS_LOG'] or 'warn' if it isn't set.
   */
...
export class Racks {
  ...
  constructor({
    ...
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('RACKS_LOG'), 'process.env[\'RACKS_LOG\']', this) ?? defaultLogLevel;
  ...
  protected async authHeaders(opts: FinalRequestOptions, schemes: { bearerCircleAPIKeyAuth?: boolean }): Promise<NullableHeaders | undefined> {
  ...
  static RacksError = Errors.RacksError;
